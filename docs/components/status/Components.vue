<template>
  <div class="component-status">
    <ul class="status-list">
      ...
      <li>
        <Icon name="review" :fill="tokens.color_warn.value" size="small" />
        <p>Under review</p>
      </li>
      <li>
        <Icon name="deprecated" :fill="tokens.color_error.value" size="small" />
        <p>Deprecated</p>
      </li>
      <li>
        <Icon name="prototype" :fill="tokens.color_info.value" size="small" />
        <p>Prototype</p>
      </li>
      <li>
        <span>—</span>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th v-if="show === 'all'">Component Type</th>
          <th>Released in</th>
          <th>Status</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(component, index) in components" :key="index" class="component">
          ...
          <td v-else>N/A</td>
          <td v-if="show === 'all'">
            <a class="rsg--link decoration-none" :href="'#/' + component.$typeName + 's'">{{
              component.$typeName
            }}</a>
          </td>
          ...
          <td v-if="component.status">
            ...
            <Icon
              ...
              name="review"
              :fill="tokens.color_warn.value"
              size="small"
            />
            <Icon
              v-if="component.status === 'prototype'"
              name="prototype"
              :fill="tokens.color_info.value"
              size="small"
            />
            <Icon
              v-if="component.status === 'deprecated'"
              name="deprecated"
              :fill="tokens.color_error.value"
              size="small"
            />
          </td>
          <td v-else>—</td>
...
export default {
  ...
  methods: {
    getComponents: function() {
      ...
      if (this.show === "all") {
        contexts = [
          { req: require.context("@/elements/", true, /\.vue$/), type: "Element" },
          { req: require.context("@/patterns/", true, /\.vue$/), type: "Pattern" },
          { req: require.context("@/templates/", true, /\.vue$/), type: "Template" },
        ]
      } else if (this.show === "elements") {
        contexts = [{ req: require.context("@/elements/", true, /\.vue$/), type: "Element" }]
      } else if (this.show === "patterns") {
        contexts = [{ req: require.context("@/patterns/", true, /\.vue$/), type: "Pattern" }]
      } else if (this.show === "templates") {
        contexts = [{ req: require.context("@/templates/", true, /\.vue$/), type: "Template" }]
      }
      ...
      contexts.forEach(context => {
        context.req
          .keys()
          .forEach(key => components.push({ ...context.req(key).default, $typeName: context.type }))
      })
    ...
    orderData: function(data) {
      ...
      return orderBy(byName, "$typeName")
    },
  },
  data() {
    return {
