<template>
  <div class="colors">
    ...
    >
      ...
      <span>{{ prop.category | toCategoryName }} Color</span>
...
export default {
  ...
  methods: {
    orderData: function(data) {
      ...
      // return orderBy(byName, "order")

      let bySuborderIndex = orderBy(data, "suborder", "asc")
      return orderBy(bySuborderIndex, "order")
    },
  },
  data() {
    return {
  ...
  filters: {
    toCategoryName(input) {
      const categoryId = input.split(":")[1]
      return `${categoryId[0].toUpperCase()}${categoryId.substring(1)}`
    },
    rgbToHex(value) {
      /**
       * Converts a decimal byte to hexadecimal characters.
      ...
      const components = value.replace(/[a-zA-Z() ]/g, "").split(",")
